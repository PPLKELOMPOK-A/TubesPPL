...
@section('styles')
<style>
    .container-bukti { max-width: 800px; margin: 30px auto; background: white; padding: 30px; border-radius: 15px; box-shadow: 0 4px 20px rgba(0,0,0,0.08); }
...
<div class="container-bukti">
    ...
    <div class="content-card">
        ...
        <div class="gallery-grid">
            @if($donasi->galeri && count((array)$donasi->galeri) > 0)
                @foreach((array)$donasi->galeri as $foto)
                <div class="gallery-item">
                    <img src="{{ asset('storage/' . $foto) }}" alt="Bukti Foto" onerror="this.src='https://via.placeholder.com/400x300?text=Foto+Dokumentasi'">
                </div>
                @endforeach
            @else
                <p style="color: gray; font-style: italic;">Tidak ada foto dokumentasi tersedia.</p>
            @endif
        </div>
    </div>
</div>
