...
<div class="max-w-7xl mx-auto px-4 py-8">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div class="flex flex-wrap items-center gap-3">
            <form action="{{ route('admin.mitra.index') }}" method="GET" class="relative">
            ...
            <a href="{{ route('admin.mitra.index', request()->except('kategori')) }}" class="px-6 py-2.5 rounded-full text-xs font-bold border transition {{ request('kategori') == null ? 'bg-[#333] text-white' : 'bg-white text-gray-600 border-gray-200' }}">Semua</a>
            @foreach(['Restoran', 'Toko', 'NGO', 'Kantin'] as $cat)
                <a href="{{ route('admin.mitra.index', array_merge(request()->all(), ['kategori' => $cat])) }}" class="px-6 py-2.5 rounded-full text-xs font-bold border transition {{ request('kategori') == $cat ? 'bg-[#333] text-white' : 'bg-white text-gray-600 border-gray-200' }}">{{ $cat }}</a>
            @endforeach
    ...
    <div class="flex gap-4 mb-8">
        <a href="{{ route('admin.mitra.index', request()->except('status')) }}" class="px-6 py-2 text-xs font-bold rounded-full transition {{ request('status') == null ? 'bg-[#D1D5DB] text-gray-800' : 'text-gray-500 hover:text-gray-800 underline' }}">Semua Mitra</a>
        <a href="{{ route('admin.mitra.index', array_merge(request()->all(), ['status' => 'aktif'])) }}" class="px-6 py-2 text-xs font-bold rounded-full transition {{ request('status') == 'aktif' ? 'bg-[#D1D5DB] text-gray-800' : 'text-gray-500 hover:text-gray-800' }}">Aktif</a>
        <a href="{{ route('admin.mitra.index', array_merge(request()->all(), ['status' => 'pengajuan'])) }}" class="px-6 py-2 text-xs font-bold rounded-full transition {{ request('status') == 'pengajuan' ? 'bg-[#D1D5DB] text-gray-800' : 'text-gray-500 hover:text-gray-800' }}">Pengajuan Baru</a>
        <a href="{{ route('admin.mitra.index', array_merge(request()->all(), ['status' => 'tidak_aktif'])) }}" class="px-6 py-2 text-xs font-bold rounded-full transition {{ request('status') == 'tidak_aktif' ? 'bg-[#D1D5DB] text-gray-800' : 'text-gray-500 hover:text-gray-800' }}">Tidak Aktif</a>
    </div>
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white border border-gray-100 p-8 rounded-[2.5rem] shadow-sm flex flex-col md:flex-row items-center gap-8">
            ...
            <div class="flex flex-col gap-3 w-full md:w-40">
                ...
                @elseif($item->status == 'pengajuan')
                    <form action="{{ route('admin.mitra.updateStatus', $item->id) }}" method="POST" class="w-full">
...
<div id="modalTambahMitra" class="fixed inset-0 z-50 hidden bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
    <div class="bg-white rounded-[2.5rem] w-full max-w-md shadow-2xl overflow-hidden">
        ...
        <form action="{{ route('admin.mitra.store') }}" method="POST" class="p-8 space-y-6">
            @csrf
            <div>
